...
const StarRating = ({ value }: { value: number }) => {
  return (
    <div className="flex gap-0.5">
      {[...Array(5)].map((_, i) => (
        <Star key={i} size={16} className={i < value ? 'text-yellow-500 fill-yellow-500' : 'text-yellow-500'} />
      ))}
    </div>
  );
};
...
const ProductFeedback = () => {
  return (
    <div className="mt-12 bg-white max-w-6xl mx-auto rounded-lg shadow-lg p-6">
      <h3 className="text-xl font-semibold mb-4">Comentarios de este producto</h3>

      <div className="flex flex-col md:flex-row justify-between gap-6 mb-6">
        {/* Calificación promedio y barras */}
        <div className="flex flex-col gap-2 w-full md:w-1/2">
          <p className="text-4xl font-bold">4.9<span className="text-gray-600 text-lg"> / 5</span></p>
          <StarRating value={5} />
          ...
          {[5, 4, 3, 2, 1].map((star) => (
            <div key={star} className="flex items-center gap-2 text-sm text-gray-600">
              ...
              <div className="flex-1 bg-gray-200 h-1 rounded overflow-hidden">
                <div className={`bg-gray-600 h-full ${star === 5 ? 'w-[90%]' : star === 4 ? 'w-[15%]' : 'w-[0%]'}`} />
              </div>
              <span className="w-6 text-end">{star === 5 ? '6' : star === 4 ? '1' : '0'}</span>
            </div>
          ))}
        </div>
